...
@dask.delayed
def daskTest(fClass, fmet, seed, sub):
    # fExtract = fclass.featureEClass()

    specificSubject = sub
    mDataList = None
    mDataList = fClass.getFeatures(specificSubject, t_min=2, t_max=3, sampling_rate=256,
                                   twoDLabels=False)
    ...
    for data_train, data_test, labels_train, labels_test, name in mDataList:
        ...
        bestLin, bestRBF, bestSigm, allResults = fmet.testSuite(
    ...
    # Saving the results
    savearray = np.array([bestResultsPerDataSet, seed,
                         specificSubject, allResultsPerDataSet], dtype=object)
    ...
    print(f"Done with seed: {seed} subject {sub} ")
    np.save(
        f"F:/PythonProjects/NietoExcercise-1/SavedResults/savedBestSeed-{seed}-\
            Subject-{specificSubject}-Date-{now_string}", savearray)
    return f"Done with seed: {seed} subject : {sub}"


fClassDict = dict()
fmetDict = dict()
resultDoneList = []
for seed in np.arange(1818, 1820):
    for sub in [1, 2, 3, 4, 5, 6, 7, 8, 9]:  # 1 ,2 as well!
        # for sub in [1]:
        np.random.seed(seed+sub)
        print(f"seed:{seed}-sub:{sub}")
        fClassDict[f"{seed},{sub}"] = fclass.featureEClass()
        fmetDict[f"{seed},{sub}"] = svmMet.SvmMets()
        resultDoneList.append(
            daskTest(fClassDict[f"{seed},{sub}"], fmetDict[f"{seed},{sub}"], seed, sub))

rDoneList = dask.compute(resultDoneList)
print(rDoneList)
...
# fClassDict = dict()
# fmetDict = dict()
# for seed in np.arange(5100,5300):
#     np.random.seed(seed)
#     for sub in [1,2,3,4,5,6,7,8,9]: #1 ,2 as well!
#     #for sub in [1]:
#         print(f"seed:{seed}-sub:{sub}")
...
# (specificSubject, t_min = 2, t_max = 3 , sampling_rate= 256,
#         twoDLabels=False)
...
#             bestLin, bestRBF, bestSigm, allResults = \
# fmetDict[f"{seed},{sub}"].testSuite(data_train, data_test, labels_train, labels_test, name)
#             allResultsPerDataSet.append(allResults)
#             if bestRBF is not None:
...
#         # seed = 4
#         # mDataList = createListOfDataMixes([[np.copy(dataHRCV),
# "dataHRCV"], [np.copy(dataHICV), "dataHICV"], [np.copy(dataCV), "dataCV"],
# [np.copy(dataWCV), "dataWCV"]], labels, seed=seed)
