...
from NNMethodsNew import NNMetsNew
...
def main():
    ...
    onlyCreateFeatures = False  # Never on NN
    useAllFeatures = True
    ...
    for sub in subjects:  #
        ...
        for createdFeature in createdFeatureList:
            print(createdFeature[1])
        print(f"Corrected EXIST doesnt matter = {correctedExists}")

        # correctedExists = True  # Never on NN loop
        # if correctedExists is False:
    ...
    for seed in np.arange(seedStart * testSize, (seedStart + 1) * testSize):
        ...
        for sub in subjects:  #
            ...
            for (
                ...
                allResults = fmetDict[f"{sub}"].testSuiteNN(
                    data_train,
                    data_test,
                    labels_train,
                    labels_test,
