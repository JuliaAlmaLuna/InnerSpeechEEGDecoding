...
for seed in np.arange(25,40):
    ...
    #for sub in [1]:
                    ...
                    for lz in [100]: #,50 10,10,25,10,,25
                        for reg in [0,1.5,3]: #[0.5,1.5,3]
                            for drp in [0,1]: #(1,2):,3
                                ...
                                eeg_model = nnM.makeModel(data_train, reg1=reg, reg2=reg, reg3=reg, dropout=drp, layerSize=lz, activation=act)
                                eeg_model.build()
                                #eeg_model.summary()
                                ...
                                dataset_losses.append([name, this_acc, act, lz, reg, drp])
            #Saving the results
        ...
        savearray = np.array([bestResultsTot, seed, specificSubject, ResultsPerDataSet], dtype=object)
        ...
        np.save(f"F:/PythonProjects/NietoExcercise-1/SavedResultsNN/savedBestSeed-{seed}-Subject-{specificSubject}-Date-{now_string}",savearray)
