...
class featureEClass():
    ...
    def createListOfDataMixes(self, dataArray, labels, order):
       ...
       # print(combos.shape)
        ...
        Returns:
            list normShufflesDataList: List of all mixes of features that have been normalized, shuffled and split into training test datasets
        """
    ...
    def getFeatures(self, subject, t_min=2, t_max=3, sampling_rate=256, twoDLabels=False):
       ...
       #dataCV = keras.utils.normalize(dataCV , axis=1, order=2)
        ...
        # mDataList = self.createListOfDataMixes([[dataHRCV, "dataHRCV"], [dataCV2, "dataCV2"], [dataWCV, "dataWCV"]], labels, order=order) #        mDataList = self.createListOfDataMixes([[dataHRCV, "dataHRCV"], [dataHICV, "dataHICV"], [dataCV, "dataCV"], [dataWCV, "dataWCV"]], labels, order=order)
        # mDataList = self.createListOfDataMixes([[dataHRCV, "dataHRCV"], [dataHICV, "dataHICV"], [dataCV, "dataCV"], [dataWCV, "dataWCV"]], labels, order=order)
        mDataList = self.createListOfDataMixes([[dataWCV, "dataWCV"], [dataCV2, "dataCV2"], [
                                               dataHRCV, "dataHRCV"]], labels, order=order)
        # mDataList = self.createListOfDataMixes([[dataWCV, "dataWCV"], [dataWCV2, "dataWCV2"] ], labels, order=order) #        mDataList = self.createListOfDataMixes([[dataHRCV, "dataHRCV"], [dataHICV, "dataHICV"], [dataCV, "dataCV"], [dataWCV, "dataWCV"]], labels, order=order)
