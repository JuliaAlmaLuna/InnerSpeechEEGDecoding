...
class featureEClass():
    ...
    def getFeatures(self, subject, t_min =2 , t_max = 3, sampling_rate = 256
       ...
       #dataCV = keras.utils.normalize(dataCV , axis=1, order=2)
        ...
        mDataList = self.createListOfDataMixes([[dataWCV, "dataWCV"],[dataCV2, "dataCV2"], [dataHRCV, "dataHRCV"] ], labels, order=order) #        mDataList = self.createListOfDataMixes([[dataHRCV, "dataHRCV"], [dataHICV, "dataHICV"], [dataCV, "dataCV"], [dataWCV, "dataWCV"]], labels, order=order)
