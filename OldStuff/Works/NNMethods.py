...
def trainTestModel2(eeg_model, data_train, data_test,
    ...
    #Training NN
    outputs = eeg_model.fit(data_train_send, labels_train, validation_split=0.2 ,callbacks=[callback], epochs=1000, verbose=False) #
    ...
    plt.title('Training and validation loss for reg val {} and dropout val {} and layersize val {}'.format(reg,drp, lz))
    plt.legend()
    plt.figure()
    ...
    plt.title('Training and validation acc for reg val {} and dropout val {} and layersize val {}'.format(reg,drp, lz))
    plt.legend()
    plt.figure()
    return outputs
