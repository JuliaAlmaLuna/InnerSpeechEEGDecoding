...
for seed in np.arange(1818, 5300):
    ...
    for sub in [1, 2, 3, 4, 5, 6, 7, 8, 9]:  # 1 ,2 as well!
        ...
        for data_train, data_test, labels_train, labels_test, name in mDataList:
            ...
            bestLin, bestRBF, bestSigm, allResults = fmetDict[f"{seed},{sub}"].testSuite(
        ...
        np.save(
            f"F:/PythonProjects/NietoExcercise-1/SavedResults/\
                savedBestSeed-{seed}-Subject-{specificSubject}-Date-{now_string}", savearray)
        ...
        # seed = 4
        # mDataList = createListOfDataMixes([[np.copy(dataHRCV), "dataHRCV"], [np.copy(dataHICV), "dataHICV"],
        # [np.copy(dataCV), "dataCV"], [np.copy(dataWCV), "dataWCV"]], labels, seed=seed)
